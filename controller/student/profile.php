<?php
class ControllerStudentProfile extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('student/profile');
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_student_profile->addStudent($this->request->post);
			...
			$this->redirect(HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . $url);
		}
	...
	public function update() {
		...
		if($this->session->data['user_id']!=$this->request->get['user_id'] || $this->session->data['group_id']!=1)
		{
			$this->redirect(HTTPS_SERVER . 'index.php?route=student/profile/update&token=' . $this->session->data['token'] . '&user_id='.$this->session->data['user_id']);
		}
		else
		{
			if (isset($this->session->data['success'])) {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
				...
				$this->model_student_profile->editStudent($this->request->get['user_id'], $this->request->post);
					
				/*
				 $student_old_subjects = "";
	  		$student_new_subjects = "";
	  		$new_subjects = (array) $this->request->post['subjects'];
	  		$old_subjects = @explode(",", $this->request->post['old_subject_ids']);
	  		$check_subjects = array_diff($new_subjects, $old_subjects);
	  		*/
				if($this->request->post['grade_year_old'] != $this->request->post['grade_year']) { // || count($check_subjects) > 0

					$this->load->model('cms/notifications');
					/*
					 $all_new_subjects = $this->model_student_profile->getSubjectsByGradeId($this->request->post['grade_year']);
					 	
	  			$all_old_subjects = $this->model_student_profile->getSubjectsByGradeId($this->request->post['grade_year_old']);

	  			foreach($new_subjects as $each_new_id) {
	  			if(empty($student_old_subjects))
	  			$student_new_subjects = $all_new_subjects[$each_new_id];
	  			else
	  			$student_new_subjects .= ", " . $all_new_subjects[$each_new_id];
	  			}

	  			foreach($old_subjects as $each_old_id) {
	  			if(empty($student_old_subjects))
	  			$student_old_subjects = $all_old_subjects[$each_old_id];
	  			else
	  			$student_old_subjects .= ", " . $all_old_subjects[$each_old_id];
	  			}

	  			print_r($old_subjects);
	  			print_r($all_old_subjects);

	  			print_r($new_subjects);
	  			print_r($all_new_subjects);

	  			print_r($this->request->post);

	  			die;*/

					$grade_years = $this->model_student_profile->getGradesAndYears();
					$old_grade_year = $grade_years[$this->request->post['grade_year_old']];
					$new_grade_year = $grade_years[$this->request->post['grade_year']];
					$change_grade = " Grade changed from $old_grade_year to $new_grade_year";

					$message = "Student changed his/her grade from <b>$old_grade_year</b> to <b>$new_grade_year</b>";
					$notification = array(
					'notification_from' => $this->session->data['user_id'],
					'notification_to' => '1',
					'subject'=> 'Student Grade changed',
					'message'=> $message 
					);
				...
				$url .= '&user_id=' . $this->session->data['user_id'];
					
				$this->redirect(HTTPS_SERVER . 'index.php?route=student/profile/update&token=' . $this->session->data['token'] . $url);
			}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $user_id) {
				$this->model_student_profile->deleteStudent($user_id);
			}
	...
	private function getList() {
		...
		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
      		);

      		$this->data['approve'] = HTTPS_SERVER . 'index.php?route=student/profile/approve&token=' . $this->session->data['token'] . $url;
      		$this->data['insert'] = HTTPS_SERVER . 'index.php?route=student/profile/insert&token=' . $this->session->data['token'] . $url;
      		$this->data['delete'] = HTTPS_SERVER . 'index.php?route=student/profile/delete&token=' . $this->session->data['token'] . $url;
			...
			'limit'                    => $this->config->get('config_admin_limit')
      		...
      		$user_total = $this->model_student_profile->getTotalStudents($data);

      		$results = $this->model_student_profile->getStudents($data);
      		...
      		foreach ($results as $result) {
				...
				'href' => HTTPS_SERVER . 'index.php?route=student/profile/update&token=' . $this->session->data['token'] . '&user_id=' . $result['user_id'] . $url
      			);
      		...
      		$this->data['sort_name'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=name' . $url;
      		$this->data['sort_email'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=c.email' . $url;
      		$this->data['sort_city'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=city' . $url;
      		$this->data['sort_subjects'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=subjects' . $url;
      		$this->data['sort_status'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=c.status' . $url;
      		$this->data['sort_approved'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=c.approved' . $url;
      		$this->data['sort_date_added'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . '&sort=c.date_added' . $url;
      		...
      		$pagination->url = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . $url . '&page={page}';
      		...
      		$this->template = 'student/profile_list.tpl';
      		$this->children = array(
			'common/header',	
			'common/footer'	
			);
	...
	private function getForm() {
		...
		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=student/profile/update&token=' . $this->session->data['token'] . $url,
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
      		);

      		if (!isset($this->request->get['user_id'])) {
      			$this->data['action'] = HTTPS_SERVER . 'index.php?route=student/profile/insert&token=' . $this->session->data['token'] . $url;
      		} else {
      			$this->data['action'] = HTTPS_SERVER . 'index.php?route=student/profile/update&token=' . $this->session->data['token'] . '&user_id=' . $this->request->get['user_id'] . $url;
      		}

      		$this->data['cancel'] = HTTPS_SERVER . 'index.php?route=student/profile&token=' . $this->session->data['token'] . $url;

      		$all_subject_ids = array();
      		$all_subjects = array();
      		if (isset($this->request->get['user_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      			$user_info = $this->model_student_profile->getStudent($this->request->get['user_id']);
      			$all_subject_ids = array_keys($this->model_student_profile->getStudentSubjects($user_info['user_id']));
      			$all_subjects = $this->model_student_profile->getSubjectsByGradeId($user_info['grades_id']);
      			//    		print_r($user_info);
      		}
      		 
      		$this->data['old_subject_ids'] = @implode(",", $all_subject_ids);
      		 
      		if (isset($this->request->post['student_status'])) {
      			$this->data['student_status_id'] = $this->request->post['student_status'];
      		} elseif (isset($user_info)) {
      			$this->data['student_status_id'] = $user_info['students_status_id'];
      		} else {
      			$this->data['student_status_id'] = '1';
      		}
      		...
      		$this->data['student_status'] = $student_status[$this->data['student_status_id']];
      		...
      		if (isset($this->request->post['postcode'])) {
      			$this->data['postcode'] = strtoupper($this->request->post['postcode']);
      		} elseif (isset($user_info)) {
      			$this->data['postcode'] = strtoupper($user_info['pcode']);
      		} else {
      			$this->data['postcode'] = '';
      		}
      		...
      		$this->template = 'student/profile_form.tpl';
      		$this->children = array(
			'common/header',	
			'common/footer'	
			);
	...
	public function subjects() {
		...
		$all_subjects = $this->model_student_profile->getSubjectsByGradeId($grade_id);

		$class = 'odd';
		foreach ($all_subjects as $subject_id => $subject_name) {
			$class = ($class == 'even' ? 'odd' : 'even'); ?>
<div class="<?php echo $class; ?>"><?php if (in_array($subject_id, $all_subject_ids)) { ?>
<input type="checkbox" name="subjects[]"
	value="<?php echo $subject_id; ?>" checked="checked" /> <?php echo $subject_name; ?>
			<?php } else { ?> <input type="checkbox" name="subjects[]"
	value="<?php echo $subject_id; ?>" /> <?php echo $subject_name; ?> <?php } ?>
</div>
			<?php }
	}
	...
	public function approve() {
		...
		if (!$this->user->hasPermission('modify', 'student/profile')) {
			...
			if (isset($this->request->post['selected'])) {
				...
				foreach ($this->request->post['selected'] as $user_id) {
					$user_info = $this->model_student_profile->getStudent($user_id);
						
					if ($user_info && !$user_info['approved']) {
						$this->model_student_profile->approve($user_id);
	...
	private function validateForm() {
		...
		if($this->request->post['username'] != $this->request->post['email']) {
			if (!isset($this->error['email']) && count($this->model_student_profile->checkStudent($this->request->post['email']))) {
