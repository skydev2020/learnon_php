...
//				$time = ($hours * 100);
  	...
  	public function getForm() {
		...
		if ($order_info) {
			...
			$this->data['entry_update_hours'] = $this->language->get('entry_update_hours');
			$this->data['button_update_hours'] = $this->language->get('button_update_hours');
...
//				print_r($invoice_data);
				...
				$this->data['invoice_date'] = $invoice_data['invoice_date'];
				
				$this->data['text_edit_invoice'] = $this->language->get('text_edit_invoice');				
				$url = "&invoice_id=".$order_info['invoice_pk'];
				$this->data['link_edit_invoice'] = HTTPS_SERVER . 'index.php?route=cms/invoices/update&token=' . $this->session->data['token'] . $url;
				...
				$this->data['student_name'] = $invoice_data['student_name'];
		
				if (isset($invoice_data)) {
					$this->data['invoice_date'] = date('Y-m-d', strtotime($invoice_data['invoice_date']));
				} else {
					$this->data['invoice_date'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['send_date'] = date('Y-m-d', strtotime($invoice_data['send_date']));
				} else {
					$this->data['send_date'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['invoice_num'] = $invoice_data['invoice_prefix']."-".$invoice_data['invoice_num'];
				} else {
					$this->data['invoice_num'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['num_of_sessions'] = $invoice_data['num_of_sessions'];
				} else {
					$this->data['num_of_sessions'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['invoice_notes'] = $invoice_data['invoice_notes'];
				} else {
					$this->data['invoice_notes'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['paid_amount'] = $invoice_data['paid_amount'];
				} else {
					$this->data['paid_amount'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['total_amount'] = $invoice_data['total_amount'];
				} else {
					$this->data['total_amount'] = "";
				}
				
				if (isset($invoice_data)) {
					$this->data['total_hours'] = $invoice_data['total_hours'];
				} else {
					$this->data['total_hours'] = "";
				}
				
				if (isset($invoice_data)) {
					if($invoice_data['pay_date']!="" && $invoice_data['pay_date']!="0000-00-00 00:00:00")
						$this->data['pay_date'] = date($this->language->get('date_format_short'), strtotime($invoice_data['pay_date']));
					else
					$this->data['pay_date'] = "";
				} else {
					$this->data['pay_date'] = "";
				...
				if (isset($invoice_data)) {
					$this->data['invoice_status'] = $invoice_data['invoice_status'];
				} else {
				...
				$this->data['column_total_hour'] = $this->language->get('column_total_hour');
				$this->data['column_hour_left'] = $this->language->get('column_hour_left');
				
				$student_packages = array();
				$student_packages = $this->model_cms_invoices->getStudentPackages($invoice_data['student_id']);
...
//				print_r($student_packages);
				...
				$package_data = $this->model_cms_packages->getInformation($order_info['package_id']);
				$package_data['package_description'] = html_entity_decode($package_data['package_description'], ENT_QUOTES, 'UTF-8');
				
				$package_data['left_hours'] = $this->format_hours($order_info['left_hours']);
								
				$this->load->model('student/packages');
				$package_data['href'] = 'javascript:void(0)';
				
				
				$package_price = $this->model_student_packages->getPackagePrice($order_info['customer_id'], $order_info['package_id']);				
				$package_data['price'] = $this->currency->format($package_price, $order_info['currency'], $order_info['value']);
				$this->data['package'] = $package_data;				
			}
			
			$this->load->model('user/students');
			$customer_data = $this->model_user_students->getStudent($order_info['customer_id']);

			$this->data['firstname'] = $customer_data['firstname'];
			$this->data['lastname'] = $customer_data['lastname'];

			if ($order_info['customer_id']) {
				$this->data['customer'] = HTTPS_SERVER . 'index.php?route=user/students/update&token=' . $this->session->data['token'] . '&user_id=' . $order_info['customer_id'];
			} else {
				$this->data['customer'] = '';
			}
  	...
  	public function generateTest() {
  		$this->load->model('sale/order');
  		echo $this->model_sale_order->generateInvoiceId('292');
  		exit;
  	}
	...
	public function generate() {
  	...
  	public function updateHours() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			// set the time into 100 Format
			$this->request->post['new_hours'] = $this->format_hours($this->request->post['new_hours'], 1);
			    		
    		$this->request->post['comment'] = "Remaining Hours Update From ".$this->request->post['old_hours']." To ".$this->request->post['new_hours'];			
			    		
    		$order_info = $this->model_sale_order->updateHours($this->request->get['order_id'], $this->request->post);
...
/*				$settings = array(
		...
		foreach ($settings as $setting) {
				...
				echo $this->config->get($setting)."<hr />";
