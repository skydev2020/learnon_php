...
class ControllerUserProfile extends Controller {  
	private $error = array();
  	...
  	{
  		$this->load->model('user/user');
		$this->model_user_user->getUpdateUsers();
  	}
  	...
  	public function insert() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->request->post['parent_firstname'] = '';
			$this->request->post['parent_lastname'] = '';
			$this->request->post['telephone'] = '';
			$this->request->post['cellphone'] = '';
			...
			$this->request->post['student_note'] = '';			
			$this->request->post['major_intersection'] = '';			
			$this->request->post['school_name'] = '';			
			$this->request->post['heard_aboutus'] = '';			
			$this->request->post['grade_year'] = 1;
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
    		...
    		// make email as a username
    		$this->request->post['username'] = $this->request->post['email'];
    		
    		$this->model_user_user->updateUserProfile($this->session->data['user_id'], $this->request->post);
			...
			$this->redirect(HTTPS_SERVER . 'index.php?route=user/profile&token=' . $this->session->data['token'] . $url);
    	}
  	...
  	private function getList() {
		...
		$this->data['column_groupname'] = $this->language->get('column_groupname');
		...
		$this->data['sort_groupname'] = HTTPS_SERVER . 'index.php?route=user/user&token=' . $this->session->data['token'] . '&sort=groupname' . $url;
	...
	private function getForm() {
		...
		$this->data['action'] = HTTPS_SERVER . 'index.php?route=user/profile/update&token=' . $this->session->data['token'] . '&user_id=' . $this->request->get['user_id'] . $url;
		  
    	$this->data['cancel'] = HTTPS_SERVER . 'index.php?route=common/home&token=' . $this->session->data['token'] . $url;
		...
		$this->template = 'user/profile_form.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
  	...
  	private function validateForm() {
    	...
    	if ((strlen(utf8_decode($this->request->post['email'])) < 1) || (strlen(utf8_decode($this->request->post['email'])) > 150)) {
      		$this->error['email'] = $this->language->get('error_email');
    	}
		 
		if(!isset($this->error['email']) && (!preg_match(EMAIL_PATTERN, $this->request->post['email']))) {
			$this->error['email'] = $this->language->get('error_invalid_email');
		}
		
		if(!isset($this->error['email'])  
			&& $this->request->post['email'] != $this->request->post['username']
			&& count($this->model_user_user->checkUser($this->request->post['email']))) {
			$this->error['email'] = $this->language->get('error_email_exsit');
		}
