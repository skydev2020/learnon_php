<?php    
class ControllerUserReportCards extends Controller { 
	private $error = array();
  
  	public function index() {
		$this->load->language('user/report_cards');
  	...
  	public function view() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_user_report_cards->editReport($this->request->get['progress_reports_id'], $this->request->post);
			...
			$this->redirect(HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . $url);
		}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['order'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . $url,
		...
		$session_total = $this->model_user_report_cards->getTotalReports($data);
		$results = $this->model_user_report_cards->getReports($data);
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => HTTPS_SERVER . 'index.php?route=user/report_cards/view&token=' . $this->session->data['token'] . '&progress_reports_id=' . $result['progress_reports_id'] . $url
			);
		...
		$this->data['sort_tutor_name'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . '&sort=tutor_name' . $url;
		$this->data['sort_student_name'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . '&sort=student_name' . $url;
		$this->data['sort_grade'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . '&sort=grade' . $url;
		$this->data['sort_subjects'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . '&sort=subjects' . $url;
		$this->data['sort_date_added'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . '&sort=date_added' . $url;
		...
		$pagination->url = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . $url . '&page={page}';
		...
		$this->template = 'user/report_cards.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
  	...
  	private function getForm($heading_title) {
		...
		$this->data['action'] = HTTPS_SERVER . 'index.php?route=user/report_cards/update&token=' . $this->session->data['token'] . '&progress_reports_id=' . $this->request->get['progress_reports_id'] . $url;
		$this->data['cancel'] = HTTPS_SERVER . 'index.php?route=user/report_cards&token=' . $this->session->data['token'] . $url;
	    
	    
		$this->data['report_info'] = $this->model_user_report_cards->getReport($this->request->get['progress_reports_id']);
		

		$this->template = 'user/view_report.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
