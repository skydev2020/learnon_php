...
class ModelLocalisationZone extends Model {
	public function addZone($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "zone SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "'"); 	
	...
	public function editZone($zone_id, $data) {
		...
		$old_zone = $this->getZone($zone_id);
		$this->db->query("UPDATE " . DB_PREFIX . "zone SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "' WHERE zone_id = '" . (int)$zone_id . "'");
		
		$this->db->query("UPDATE " . DB_PREFIX . "user_info SET state = '" . $this->db->escape($data['code']) . "' WHERE state = '" . $old_zone["code"] . "'");
	...
	public function getAllZones() {
		$query = $this->db->query("SELECT z.name, z.zone_id, z.code FROM " . DB_PREFIX . "zone z ORDER BY z.zone_id");
