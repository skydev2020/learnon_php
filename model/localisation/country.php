...
class ModelLocalisationCountry extends Model {
	public function addCountry($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "country SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "'");
	...
	public function editCountry($country_id, $data) {
	 /*	$this->db->query("UPDATE " . DB_PREFIX . "country SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', iso_code_2 = '" . $this->db->escape($data['iso_code_2']) . "', iso_code_3 = '" . $this->db->escape($data['iso_code_3']) . "', address_format = '" . $this->db->escape($data['address_format']) . "' , postcode_required = '" . (int)$data['postcode_required'] . "' WHERE country_id = '" . (int)$country_id . "'");*/
		...
		$old_country = $this->getCountry($country_id);
		
		$this->db->query("UPDATE " . DB_PREFIX . "country SET status = '" . (int)$data['status'] . "', name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "' WHERE country_id = '" . (int)$country_id . "'");
		$this->db->query("UPDATE " . DB_PREFIX . "user_info SET country = '" . $this->db->escape($data['name']) . "' WHERE country = '" . $old_country["name"] . "'");
		$this->cache->delete('country');
	}
