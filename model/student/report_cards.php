<?php
class ModelStudentReportCards extends Model {
	var $user_group_id = 1;
	
	
	public function getReport($progress_reports_id) {
		$query = $this->db->query("SELECT s.*, CONCAT(t.firstname, ' ', t.lastname) AS tutor_name FROM " . DB_PREFIX . "progress_reports s LEFT JOIN " . DB_PREFIX . "user t ON s.tutors_id = t.user_id WHERE s.progress_reports_id = '$progress_reports_id' AND s.students_id = '".$this->session->data['user_id']."' ");
		return $query->row;
	}
		
	public function getReports($data = array()) {
		$sql = "SELECT s.*, CONCAT(t.firstname, ' ', t.lastname) AS tutor_name FROM " . DB_PREFIX . "progress_reports s LEFT JOIN " . DB_PREFIX . "user t ON s.tutors_id = t.user_id ";
		...
		$sql .= " WHERE s.students_id = '".$this->session->data['user_id']."' ";
		if ($implode) {
	...
	public function getTotalReports($data = array()) {
      	$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "progress_reports s LEFT JOIN " . DB_PREFIX . "user t ON s.tutors_id = t.user_id ";
