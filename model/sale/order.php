...
class ModelSaleOrder extends Model {
	...
	public function getPackages($data) {
			...
			$sql = 'SELECT * , (SELECT CONCAT_WS(\' \',firstname,lastname) as name from user where user_id = o.customer_id) as name FROM `order` o, packages p WHERE o.package_id = p.package_id and o.order_status_id = 5 and o.package_id <> 0 ';
...
/*		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND order_id = '" . (int)$order_id . "'");
	...
	public function updateHours($order_id, $data) {
		
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET left_hours = '" . $data['new_hours'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");		
		...
		if ($data['append']) {
      	...
      	if ($data['notify']) {
			...
			if ($order_query->num_rows) {
				...
				$message .= html_entity_decode($order_query->row['store_url'] . 'index.php?route=student/invoice/view&invoice_id=' . $order_query->row['invoice_pk'], ENT_QUOTES, 'UTF-8') . "\n\n";
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.package_id, o.invoice_pk, o.payment_method, o.invoice_id, o.invoice_prefix, o.total_hours, o.total, CONCAT(o.firstname, ' ', o.lastname, ' ( ', o.customer_id,' )') AS name, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.date_added, o.total, o.currency, o.value FROM `" . DB_PREFIX . "order` o WHERE 1=1";
	...
	public function generateInvoiceId($order_id) {
		...
		*/
		...
		/*if ($this->config->get('config_invoice_no')) {
			$invoice_id = $this->config->get('config_invoice_no')+1;
		} else {
			$invoice_id = 1;
		}*/
		// end
