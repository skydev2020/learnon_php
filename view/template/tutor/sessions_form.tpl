    ...
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
      <table class="form">
        <tr>
          ...
          <td>
			...
			<?php foreach($students as $student){
				if($student['tutors_to_students_id']==$tutors_to_students_id){ ?>
			<option value="<?=$student['tutors_to_students_id']?>" selected="selected"><?=$student['student_name']?></option>
			<?php }else{ ?>
			<option value="<?=$student['tutors_to_students_id']?>"><?=$student['student_name']?></option>	 
			<?php }}?>
			</select>
        ...
        <tr>
          ...
          <td><textarea name="session_notes" cols="50" rows="10"><?php echo $session_notes; ?></textarea></td>
        </tr>
      </table>
    </form>
  </div>
