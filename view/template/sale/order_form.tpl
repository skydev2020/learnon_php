...
<div class="box">
  ...
  <div class="content">
    <div style="display: inline-block; width: 100%;">
      <div class="vtabs"><a tab="#tab_order"><?php echo $tab_order; ?></a><a tab="#tab_product"><?php echo $tab_product; ?></a><a tab="#tab_history"><?php echo $tab_history; ?></a></div>
      ...
      <div id="tab_product" class="vtabs_page">
        <table id="product" class="list">
          <thead>
            <tr>
              <td class="left" colspan="3"><?php echo $column_product; ?></td>
              <td class="right"><?php echo $column_total_hours; ?></td>
              <td class="right"><?php echo $column_left_hours; ?></td>
              <td class="right"><?php echo $column_total; ?></td>
            </tr>
          </thead>
          <tbody id="package">
            <tr>
              <td class="left" colspan="3"><a href="<?php echo $package['href']; ?>"><?php echo $package['package_name']; ?></a><br />
			  <?php echo $package['package_description']; ?></td>
			  <td class="right"><?php echo $package['hours']; ?></td>
			  <td class="right"><?php echo $package['left_hours']; ?></td>
              <td class="right"><?php echo $package['price']; ?></td>
            </tr>
          </tbody>
          <tbody id="totals">
            <?php foreach ($totals as $key => $tot) { ?>
            <tr<?php if($key != (count($totals)-1) && $key != 0) echo ' class="taxes"'; // TU ?>>
              <td colspan="5" class="right"><?php echo $tot['title']; ?></td>
      ...
      <div id="tab_history" class="vtabs_page">
        ...
        <table class="form">
          <tr>
            <td><?php echo $entry_update_hours; ?> : <span class="help">(e.g 12:30, 12.30, 1:45, 1.45)</span> </td>
            <td><input type="text" id="new_hours" name="new_hours" value="" size="10" /> &nbsp; <a onclick="update_hours();" id="update_hours_button" class="button"><span><?php echo $button_update_hours; ?></span></a>
            <input type="hidden" id="old_hours" name="old_hours" value="<?php echo $package['left_hours']; ?>" size="10" /></td>
          </tr>
          <tr>
	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.success && $('input[name=\'append\']').attr('checked')) {
				...
				html += '        <td class="left"></td>'; //' + data.notify + '
				html += '      </tr>';
				html += '    </tbody>';
...
function update_hours() {
	$.ajax({
		type: 'POST',
		url: 'index.php?route=sale/order/updateHours&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		dataType: 'json',
		data: 'order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&old_hours=' + encodeURIComponent($('input[name=\'old_hours\']').val()) + '&new_hours=' + encodeURIComponent($('input[name=\'new_hours\']').val())+ '&append=' + encodeURIComponent($('input[name=\'append\']').attr('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()),
		beforeSend: function() {
			$('.success, .warning').remove();
			$('#history_button').attr('disabled', 'disabled');
			$('#tab_history .form').before('<div class="attention"><img src="view/image/loading_1.gif" alt="" /> <?php echo $text_wait; ?></div>');
