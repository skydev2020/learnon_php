    ...
    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
      <table class="form">
        <tr>
          ...
          <td><input type="text" name="name" id="expense_name" value="<?php echo $name; ?>" /> OR 
          	<select id="all_expenses" onchange="pick_this()">
          		<option value=""><?=$text_select?></option>
          		<?php foreach($all_expenses as $each_expense) {?>
          		<option value="<?=$each_expense?>"><?=$each_expense?></option>
          		<?php } ?>
          	</select>
            <?php if ($error_name) { ?>
        ...
        <tr>
          <td valign="top"><?php echo $entry_detail; ?></td>
